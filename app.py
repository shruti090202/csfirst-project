...
def get_commit_activity(owner, repo, max_retries=3):
    ...
    for attempt in range(max_retries):
        ...
        if response.status_code == 200:
            return response.json(), None
        elif response.status_code == 202 and attempt < max_retries - 1:
            # If we get a 202, wait briefly and retry
            time.sleep(1)
            continue
        elif response.status_code == 202:
            # If we still get 202 after retries, return empty data
            return [], None
        else:
            # For other errors, use the standard handler
            return handle_api_response(response)
    
    # Fallback in case the loop completes without a return
    return [], None
...
def analyze():
    ...
    commit_data = {
        'total_commits': sum(week['total'] for week in commit_activity) if commit_activity else 0,
        'weekly_average': round(sum(week['total'] for week in commit_activity) / len(commit_activity), 2) if commit_activity and len(commit_activity) > 0 else 0,
        'recent_weeks': commit_activity[-4:] if commit_activity else []
    }
